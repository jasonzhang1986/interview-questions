...
Dalvik VM 在 Android 2.2 版本是一次飞跃，但 Dalvik 虚拟机从 DEX（ Dalvik  Executable ）格式的文件中读取指令与数据，进行解释运行，JIT 在运行时将字节码翻译为本地机器指令，运行一次就要编译一次，性能还是很低。于是 Android4.4 版本支持了 ART 模式，并在 Android5.0 正式将编译模式由 ART 取代 Dalvik VM 成为默认选项。

ART 采用预编译 AOT（ahead-of-time）技术，在应用程序安装时，将程序代码一次性转化为机器语言，省去了程序在运行时每次编译的时间，加快了程序运行速度，使得电量消耗更少，系统更加流畅。
